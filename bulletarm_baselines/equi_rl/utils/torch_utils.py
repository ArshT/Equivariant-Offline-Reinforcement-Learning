...
ExpertTransitionGoal = collections.namedtuple('ExpertTransitionGoal', 'state obs action reward next_state next_obs done step_left expert goal')
...
def perturb_new(current_image, next_image, dxy, set_theta_zero=False, set_trans_zero=False,theta=None,trans=None):
    image_size = current_image.shape[-2:]

    # Compute random rigid transform.
    if theta is None and trans is None:
        theta, trans, pivot = get_random_image_transform_params(image_size)
    else:
        _,_,pivot = get_random_image_transform_params(image_size)
...
def perturbVec(current_state, next_state, dxy, set_theta_zero=False, set_trans_zero=False):
    ...
    n_pose = (current_state.shape[0] - 1) // 5
    ...
    for i in range(n_pose):
        ...
        scaled_current_cos_sin_theta = current_state[1+i*4+3:1+i*4+5]
        scaled_aug_cos_sin_theta = rot.dot(scaled_current_cos_sin_theta)
        aug_current_state[1 + i * 4 + 3: 1 + i * 4 + 5] = scaled_aug_cos_sin_theta

        scaled_next_cos_sin_theta = next_state[1+i*4+3:1+i*4+5]
        scaled_aug_next_cos_sin_theta = rot.dot(scaled_next_cos_sin_theta)
        aug_next_state[1 + i * 4 + 3: 1 + i * 4 + 5] = scaled_aug_next_cos_sin_theta
...
def augmentTransitionSO2Controlled(d,theta):
    obs, next_obs, dxy, transform_params = perturb_new(d.obs[0].copy(),
                                                   d.next_obs[0].copy(),
                                                   d.action[1:3].copy(),
                                                   set_trans_zero=True,
                                                   theta=theta,
...
def augmentTransitionSO2ControlledAction(d,theta):
    obs, next_obs, dxy, transform_params = perturb_new(d.obs[0].copy(),
                                                   d.next_obs[0].copy(),
                                                   d.action[1:3].copy(),
                                                   set_trans_zero=True,
    ...
    action[1] = dxy[0]
    action[2] = dxy[1]
    return ExpertTransition(d.state, d.obs, action, d.reward, d.next_state,
                            next_obs, d.done, d.step_left, d.expert)
...
def augmentTransitionSE2(d):
    ...
    obs = obs.reshape(1, *obs.shape)
    next_obs = next_obs.reshape(1, *next_obs.shape)
    action = torch.Tensor(d.action).clone()
    action[1] = dxy[0]
    action[2] = dxy[1]
    return ExpertTransition(d.state, obs, action, d.reward, d.next_state,
                            next_obs, d.done, d.step_left, d.expert)
...
def augmentTransitionSE2_torch(d):
    obs, next_obs, dxy, transform_params = perturb(d.obs[0].copy(),
                                                    d.next_obs[0].copy(),
                                                    d.action[1:3].copy(),
                                                    set_trans_zero=True)
    try:
        ...
        print(d.action.get_device())
        obs, next_obs, dxy, transform_params = perturb_new(d.obs[0].copy(),
                                                    d.next_obs[0].copy(),
                                                    d.action[1:3].clone(),
                                                    set_trans_zero=True)
        d.action = d.action.to("cpu")
    ...
    try:
        return ExpertTransitionGoal(d.state, obs, action, d.reward, d.next_state,
                        next_obs, d.done, d.step_left, d.expert,d.goal)
    except:
        return ExpertTransition(d.state, obs, action, d.reward, d.next_state,
                        next_obs, d.done, d.step_left, d.expert)
...
def augmentTransitionSO2Vec_torch(d):
    obs, next_obs, dxy, transform_params = perturbVec(d.obs.copy(),
                                                      d.next_obs.copy(),
                                                      d.action[1:3].copy(),
                                                      set_trans_zero=True)
...
def normalizeTransitionGoal(d: ExpertTransitionGoal):
    obs = np.clip(d.obs, 0, 0.32)
    obs = obs/0.4*255
    obs = obs.astype(np.uint8)
    ...
    return ExpertTransitionGoal(d.state, obs, d.action, d.reward, d.next_state, next_obs, d.done, d.step_left, d.expert, d.goal)
...
def augmentBatch(batch, aug_t, aug_n):

    new_batch = []
    for i, d in enumerate(batch):
        new_batch.append(augmentTransitionSE2_torch(d))

    return new_batch


def augmentBatchVec(batch, aug_t, aug_n):

    new_batch = []
    for i, d in enumerate(batch):
        new_batch.append(augmentTransitionSO2Vec_torch(d))

    return new_batch



def augmentBatchNx(batch, aug_t, aug_n):

    new_batch = []
    for i, d in enumerate(batch):
        for new_theta in np.linspace(0, 2 * np.pi, aug_n):
            new_d = augmentTransitionSO2Controlled(d, new_theta)
            new_batch.append(augmentTransitionSE2_torch(new_d))
