...
class QLearningBuffer:
    ...
    def saveBuffer(self,filename):
        with open(filename+'.pkl', 'wb') as f:
            pickle.dump(self.getSaveState(), f)
    
    def loadBuffer(self,filename):
        with open(filename, 'rb') as f:
            save_state = pickle.load(f)
        self.loadFromState(save_state)
...
class QLearningBufferExpert(QLearningBuffer):
    ...
    def sample(self, batch_size):
        ...
        non_expert_mask = np.ones(self.__len__(), dtype=bool)
    ...
    def loadFromState(self, save_state):
        ...
        for i in range(len(self._storage)):
            if self._storage[i].expert:
                self._expert_idx.append(i)
