...
class CQL_SAC_2(A2CBase):
    ...
    def calcCriticLoss(self):
        ...
        temp_obs = obs.unsqueeze(1).repeat(1, num_repeat, 1,1,1).view(obs.shape[0] * num_repeat, obs.shape[1],obs.shape[2],obs.shape[3])
        temp_next_obs = next_obs.unsqueeze(1).repeat(1, num_repeat, 1,1,1).view(next_obs.shape[0] * num_repeat, next_obs.shape[1],next_obs.shape[2],next_obs.shape[3])
    ...
    def _compute_random_values(self, obs, actions):
        ...
        random_values1 = torch.pow(random_values1, 2)
        random_values2 = torch.pow(random_values2, 2)
