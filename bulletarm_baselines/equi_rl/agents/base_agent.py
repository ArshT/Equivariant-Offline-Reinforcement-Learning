...
class BaseAgent:
    ...
    def _loadBatchToDevice(self, batch):
        ...
        try:
            ...
            for d in batch:
                goals.append(d.goal)
            goals_tensor = torch.tensor(np.stack(goals)).long().to(self.device)
            self.loss_calc_dict['goals'] = goals_tensor

            return states_tensor, obs_tensor, action_tensor, rewards_tensor, next_states_tensor, \
                   next_obs_tensor, non_final_masks, step_lefts_tensor, is_experts_tensor, goals_tensor
    ...
    def _loadLossCalcDict(self):
        ...
        try:
            goals = self.loss_calc_dict['goals']
