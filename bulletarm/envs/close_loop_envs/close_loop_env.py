...
class CloseLoopEnv(BaseEnv):
  ...
  def _getVecObservation(self):
    ...
    scaled_gripper_cos_rz, scaled_gripper_sin_rz = self._scaleCosSinRz(gripper_rz)
    ###########################################################################################
    ...
    for i in range(obj_poses.shape[0]):
      scaled_obj_poses.append(
        ...
        np.concatenate([self._scalePos(obj_poses[i, :3]), np.array([self._scaleCosSinRz(obj_poses[i, 3])[0]]), np.array([self._scaleCosSinRz(obj_poses[i, 3])[1]])]))
        ###########################################################################################
    ...
    ###########################################################################################
    # obs = np.concatenate([np.array([gripper_state]), scaled_gripper_pos, np.array([scaled_gripper_rz]), scaled_obj_poses])
    obs = np.concatenate([np.array([gripper_state]), scaled_gripper_pos, np.array([scaled_gripper_cos_rz]),np.array([scaled_gripper_sin_rz]), scaled_obj_poses])
    ###########################################################################################
    
    # print("Vector Observation: ", obs.shape)
    return obs
